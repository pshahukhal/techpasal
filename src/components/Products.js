...
export default function Products({product}){
  return <ProductConsumer>
  {
    ...
    return (
      ...
      <Link to={`/productdetail/${product.id}`} onClick={() => setSingleProduct(product.id) }>
...
.card{
  box-shadow: 4px 6px 5px 0px rgba(186,224,245,1);
  transition: var(--mainTransition);
  height:100%;
}
.card:hover{
  box-shadow: 8px 8px 5px 0.5px rgba(186,224,245,1);
  cursor: pointer;
}
.card-img-top{
  transition: var(--mainTransition);
